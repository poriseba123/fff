        ...
        <div class="form-body">
            <div class="form-group">
                ...
                <div class="col-md-6">
                    ...
                    <input type="hidden" name="chamber_id" id="chamber_id" value="<?=$model->id?>">
                </div>
            </div>
        </div>
        <div class="form-body">
            ...
            <div class="form-group">
                ...
                <div class="col-md-9">
                    ...
                    foreach ($day_master as $key => $val) {
                        $doc_chamber_time= app\models\DoctorChamberTime::find()->where(["doctor_chamber_id"=>$model->id,"day_master_id"=>$val->id])->all();
                           $checked="";  
                           $active="";
                        if(count($doc_chamber_time)>0){
                                    $checked="checked";
                                    $active="active";
                                }
                    ?>
				...
				<span class="glyphicon glyphicon-ok"></span>&nbsp;<?=$val->day?>
                        ...
                        <div class="time <?='day_master_time_'.$val->id?>" style="margin-bottom:5px;">
                            <?php
                                if(count($doc_chamber_time)>0){
                                    foreach ($doc_chamber_time as $key => $value) {
                                      $count=count($doc_chamber_time);
                            ?>
                            <div class="row <?='each_time_'.$val->id.'_'.$key?>">
                                <div class="col-md-4">
                                    <div class='input-group date timepicker'>
                                        <input type='text' class="form-control" name="start_time[<?=$val->id?>][]" value="<?=$value->start_time?>"/>
                                ...
                                <div class="col-md-4">
                                    <div class='input-group date timepicker'>
                                        <input type='text' class="form-control" name="end_time[<?=$val->id?>][]" value="<?=$value->end_time?>"/>
                                        <span class="input-group-addon">
                                            <span class="glyphicon glyphicon-time"></span>
                                        </span>
                                    </div>
                                ...
                                <div class="col-md-4">
                                    <div class="btn-group btn-group-solid">
                                        ...
                                        if($key==0){
                                        ?>
                                        <button type="button" class="btn btn-success" style="font-size:17px;" onclick="addTime(<?=$val->id?>,<?=$count?>);">
                                            + ADD MORE
                                        </button>
                                        <?php }else{ ?>
                                        <button type="button" class="btn btn-danger" style="font-size:17px;" onclick="removeRow(<?=$val->id?>,<?=$key?>)">X</button>
                                        <?php } ?>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row <?='each_time_'.$val->id.'_0'?>">
                                ...
                                <div class="col-md-4">
                                    <div class="btn-group btn-group-solid">
                                        <button type="button" class="btn btn-success" style="font-size:17px;" onclick="addTime(<?=$val->id?>,'1');">
                                            + ADD MORE
                                        </button>
                                    </div>
                                </div>
