...
class OrphanehomeController extends AdminController {
        ...
        $gridColumns = [
            ...
            [
                ...
                'urlCreator' => function($action, $model, $key, $index) {
                    switch ($action) {
                        case "view":
                            return Url::to(['orphanehome/view', 'id' => $model->id]);
                            break;
                        case "update":
                            return Url::to(['orphanehome/update', 'id' => $model->id]);
                            break;
                        case "delete":
                            return Url::to(['orphanehome/delete', 'id' => $model->id]);
                            break;
                    }
                },
...
//                                'before' => Html::a('<i class="glyphicon glyphicon-plus"></i> Add new', ['create'], ['class' => 'btn btn-success']),
    ...
    public function actionCreateajax() {
        if (Yii::$app->request->isAjax) {
            ...
            $time_error = false;
            $resp['checkbox'] = true;
            $all_postdata_arr = $_POST['Orphanehome'];
            ...
            if ($model->load(Yii::$app->request->post())) {
                ...
                if (isset($img) && $img->error == 0) {
                    ...
                    if (!in_array(end($ext), $allow)) {
                        ...
                        $path = Yii::$app->basePath . '/uploads/orphanehome/original/' . $imgName;
                        $img->saveAs($path);
                        $this->resizeImage('orphanehome', $imgName);
                        $model->image = $imgName;
                    }
                }
                //$model->status =1;
                ...
                if ($model->validate() && $phone_error == false && $imgError == 0) {
                    ...
                    $model->save(false);
                    $resp['flag'] = true;
                    $resp['url'] = Url::to(['orphanehome/index']);
                    $resp['msg'] = "Orphane age home successfully created";
                } else {
    ...
    public function actionUpdateajax() {
        if (Yii::$app->request->isAjax) {
            ...
            $model = Orphanehome::findOne($oldage_id);
            ...
            if ($model->load(Yii::$app->request->post())) {
                ...
                if ($model->validate() && $phone_error == false && $imgError == 0) {
                    ...
                    $resp['flag'] = true;
                    $resp['url'] = Url::to(['orphanehome/index']);
                    $resp['msg'] = "Orphane home successfully updated";
                } else {
                    $resp['errors'] = $model->getErrors();
                }
            }
    ...
    public function actionDelete($id) {
        $chamber = Orphanehome::findOne($id);
        $chamber->status = 3;
        $chamber->save(false);
...
//                        return $this->redirect(['index']);
    ...
    protected function findModel($id) {
        if (($model = Orphanehome::findOne($id)) !== null) {
            return $model;
        } else {
