...
class StateController extends AdminController {
    ...
    public function column() {
        ...
        $updateMsg = 'Update';
		$rowsall=[];
		$country_list = \app\models\Countries::find()->select('name, id')->where('status <> \'3\'')->all();
        ...
        $gridColumns = [
            ...
            [
				...
				'value' => function($data){
					 $country_list = \app\models\Countries::find()->select('name, id')->where(["id" => $data->country_id])->all();
					 foreach($country_list as $model){
					  $rows[] = array_filter($model->attributes);
				   }
					 $listData = ArrayHelper::map($rows, 'id', 'name');
					
					 return $listData[$data->country_id];
				},
            ...
            [
                ...
                'urlCreator' => function($action, $model, $key, $index) {
                    switch ($action) {
                        case "view":
                            return Url::to(['state/view', 'id' => $model->id]);
                            break;
                        case "update":
                            return Url::to(['state/update', 'id' => $model->id]);
                            break;
                        case "delete":
                            return Url::to(['state/delete', 'id' => $model->id]);
                            break;
                    }
                },
                        ...
                        if (Yii::$app->request->isAjax) {
                            ...
                            if ($model->load(Yii::$app->request->post())) {
								if ($model->validate()) {
                                    $model->save(false);
                                    $resp['flag'] = true;
                                    $resp['url'] = Url::to(['state/index']);
                                    $resp['msg'] = "State successfully created";
                                } else {
