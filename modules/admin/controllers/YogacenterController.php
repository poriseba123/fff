...
class YogacenterController extends AdminController {
        ...
        $gridColumns = [
            ...
            [
                ...
                'urlCreator' => function($action, $model, $key, $index) {
                    switch ($action) {
                        case "view":
                            return Url::to(['yogacenter/view', 'id' => $model->id]);
                            break;
                        case "update":
                            return Url::to(['yogacenter/update', 'id' => $model->id]);
                            break;
                        case "delete":
                            return Url::to(['yogacenter/delete', 'id' => $model->id]);
                            break;
                    }
                },
            ...
            if ($model->load(Yii::$app->request->post())) {
                ...
                if (isset($img) && $img->error == 0) {
                    ...
                    if (!in_array(end($ext), $allow)) {
                        ...
                        $path = Yii::$app->basePath . '/uploads/yogacenter/original/' . $imgName;
                        $img->saveAs($path);
                        $this->resizeImage('yogacenter', $imgName);
                        $model->image = $imgName;
                    }
                }
                //$model->status =1;
...
//                    die();
                    $model->save(false);
                    $resp['flag'] = true;
                    $resp['url'] = Url::to(['yogacenter/index']);
                    $resp['msg'] = "Yoga Centre successfully created";
                } else {
    ...
    public function actionUpdateajax() {
        if (Yii::$app->request->isAjax) {
            ...
            $model = YogaCenter::findOne($yoga_center_id);
            ...
            if ($model->load(Yii::$app->request->post())) {
                ...
                if ($model->validate() && $phone_error == false && $imgError == 0) {
                    ...
                    $resp['flag'] = true;
                    $resp['url'] = Url::to(['yogacenter/index']);
                    $resp['msg'] = "Yoga Centre successfully updated";
                } else {
                    $resp['errors'] = $model->getErrors();
                }
            }
...
//                        return $this->redirect(['index']);
    ...
    protected function findModel($id) {
        if (($model = YogaCenter::findOne($id)) !== null) {
            return $model;
        } else {
