...
class DistrictController extends AdminController {
    ...
    public function column() {
        ...
        $updateMsg = 'Update';
		$rowsall=[];
		$state_list = \app\models\States::find()->select('name, id')->where('status <> \'3\'')->all();
					 foreach($state_list as $model){
        ...
        $gridColumns = [
            ...
            [
                ...
                'attribute' => 'state_id',
				'value' => function($data){
					 $country_list = \app\models\States::find()->select('name, id')->where(["id" => $data->state_id])->all();
					 foreach($country_list as $model){
					  $rows[] = array_filter($model->attributes);
				   }
					 $listData = ArrayHelper::map($rows, 'id', 'name');
					
					 return $listData[$data->state_id];
				},
            ...
            [
                ...
                'urlCreator' => function($action, $model, $key, $index) {
                    switch ($action) {
                        case "view":
                            return Url::to(['district/view', 'id' => $model->id]);
                            break;
                        case "update":
                            return Url::to(['district/update', 'id' => $model->id]);
                            break;
                        case "delete":
                            return Url::to(['district/delete', 'id' => $model->id]);
                            break;
                    }
                },
...
//                                'before' => Html::a('<i class="glyphicon glyphicon-plus"></i> Add new', ['create'], ['class' => 'btn btn-success']),
                        ...
                        if (Yii::$app->request->isAjax) {
                            ...
                            if ($model->load(Yii::$app->request->post())) {
								if ($model->validate()) {
                                    $model->save(false);
                                    $resp['flag'] = true;
                                    $resp['url'] = Url::to(['district/index']);
                                    $resp['msg'] = "District successfully created";
                                } else {
    ...
    public function actionUpdateajax() {
                        if (Yii::$app->request->isAjax) {
                            ...
                            $resp = [];
							$resp['flag'] = false;
							$model= Districts::findOne($district_id);
