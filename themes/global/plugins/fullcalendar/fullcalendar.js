...
/* A utility class for rendering <td> rows.
----------------------------------------------------------------------------------------------------------------------*/
...
var RowRenderer = Class.extend({
	...
	rowHtml: function(rowType, row) {
		...
		return '<td>' + rowCellHtml + '</tr>';
	},


	// Applies the "intro" and "outro" HTML to the given cells.
	// Intro means the leftmost cell when the calendar is LTR and the rightmost cell when RTL. Vice-versa for outro.
	// `cells` can be an HTML string of <td>'s or a jQuery <td> element
	// `row` is an optional row number.
...
DayGrid.mixin({
	...
	computeRowLevelLimit: function(row) {
		...
		// Reveal one level <td> at a time and stop when we find one out of bounds
	...
	limitRow: function(row, levelLimit) {
		...
		var limitedNodes; // array of temporarily hidden level <td> and segment <td> DOM nodes
		var i, seg;
		...
		if (levelLimit && levelLimit < rowStruct.segLevels.length) { // is it actually over the limit?
			...
			limitedNodes = rowStruct.tbodyEl.children().slice(levelLimit) // get level <td> elements past the limit
